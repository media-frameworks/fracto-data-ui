...
const HistoryWrapper = styled(CoolStyles.Block)`
   margin: 1rem 0;
`
...
export class FieldGenerator extends Component {
   ...
   componentDidMount() {
      FractoDataLoader.load_tile_set_async(BIN_VERB_READY, result => {
         ...
         this.setState({ready_loading: false});
      });
   ...
   on_render_detail = (tile, detail_width_px) => {
      const {sort_type, sort_extra, tile_option, run_history} = this.state
      const type_options = [
      ...
      return <CoolStyles.Block>
         ...
         <HistoryWrapper><FractoTileRunHistory
            width_px={detail_width_px}
            history_items={run_history}/>
         </HistoryWrapper>
      </CoolStyles.Block>
   }
   ...
   generate_tile = (tile, cb) => {
      ...
      if (!run_history.length) {
         const history_item = FractoTileRunHistory.format_history_item(null, "start", '')
         run_history.push(history_item)
      }
      const tile_points = FractoTileGenerate.prepare_tile()
      FractoTileGenerate.generate_tile(tile, tile_points, response => {
         const history_item = FractoTileRunHistory.format_history_item(tile, "generate", response)
         run_history.push(history_item)
